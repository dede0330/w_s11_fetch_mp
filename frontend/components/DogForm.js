...
export default function DogForm({ dog, getDogs, reset }) {
  ...
  const postDog = () => {
    ...
    fetch("api/dogs", { method: "POST", body: JSON.stringify(values), headers: new Headers({ "Content-Type": "application/json" })})
      .then((response) => {
  ...
  const putDog = () => {
    ...
    fetch(`api/dogs/${values.id}`, { method: "PUT", body: JSON.stringify(values), headers: new Headers({ "Content-Type": "application/json" })})
    .then((response) => {
